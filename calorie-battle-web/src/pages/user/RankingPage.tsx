  ...
  switch (rank) {
    case 1:
      return {
        ...
        icon: <CrownOutlined style={{ color: '#ffd700', fontSize: 24 }} />,
      };
    case 2:
      return {
        color: '#c0c0c0',
        bgColor: 'linear-gradient(135deg, #f5f5f5 0%, #e0e0e0 100%)',
        icon: <SafetyCertificateOutlined style={{ color: '#c0c0c0', fontSize: 22 }} />,
      };
    case 3:
      return {
        color: '#cd7f32',
        bgColor: 'linear-gradient(135deg, #fff7e6 0%, #ffe7ba 100%)',
        icon: <TrophyOutlined style={{ color: '#cd7f32', fontSize: 20 }} />,
      };
    default:
      return {
        color: '#999',
  ...
  return (
    <div
      style={{
        ...
        border: rank === 1 ? '2px solid #ffd700' : '1px solid #f0f0f0',
        position: 'relative',
    ...
    >
      ...
      <Avatar
        ...
        style={{
          marginTop: 16,
          border: rank === 1 ? '3px solid #ffd700' : '2px solid #d9d9d9',
          boxShadow: rank === 1 ? '0 4px 12px rgba(255,215,0,0.3)' : 'none',
        }}
      />
      <Text
        strong
...
const RankingPage: React.FC = () => {
  ...
  // 当前用户排名（如果不在列表中，从列表外查找）
  const myRanking = rankingList.find(
    (item) => item.is_current_user || item.user_id === user?.id
  );
    ...
    <div className="page-container">
      ...
      {topThree.length > 0 && (
        <Card
          ...
          bodyStyle={{ padding: '32px 16px 16px' }}
        >
          <div
            style={{
              display: 'flex',
          ...
          >
            {/* 排列顺序: 第2名 | 第1名 | 第3名 */}
            {[topThree.find((i) => i.rank === 2), topThree.find((i) => i.rank === 1), topThree.find((i) => i.rank === 3)]
              .filter(Boolean)
              .map((item) => (
                <div
                  key={item!.rank}
      ...
      <Card style={{ borderRadius: 12, border: 'none', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
        <List
          loading={isLoading}
          locale={{
            emptyText: <Empty description="暂无排名数据" />,
          }}
          dataSource={restList}
          renderItem={(item) => {
            ...
            return (
              <List.Item
                style={{
                  ...
                  borderRadius: 8,
                  marginBottom: 4,
                  border: isCurrentUser ? '1px solid #91caff' : 'none',
                }}
              >
                <List.Item.Meta
                  avatar={
                  ...
                  description={
                    <Text type="secondary" style={{ fontSize: 13 }}>
                      累计获得 {formatPoints(item.total_earned)} 积分
                    </Text>
                  }
                />
                <div style={{ textAlign: 'right' }}>
                  <Text strong style={{ fontSize: 18, color: rankStyle.color }}>
                    {formatPoints(item.total_points)}
                  </Text>
                  <br />
                  <Text type="secondary" style={{ fontSize: 12 }}>
