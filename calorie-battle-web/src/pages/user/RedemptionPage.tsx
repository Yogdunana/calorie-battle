...
const RedemptionPage: React.FC = () => {
  ...
  const { data: itemsData, isLoading: itemsLoading } = useQuery({
    queryKey: ['redemptionItems'],
    queryFn: () => redemptionApi.getItems(),
  });

  // 获取兑换记录
  const { data: redemptionsData, isLoading: redemptionsLoading } = useQuery({
    queryKey: ['myRedemptions', page, pageSize],
    queryFn: () => redemptionApi.getMyRedemptions({ page, pageSize }),
  });
    ...
    Modal.confirm({
      ...
      content: (
        <div>
          <p>商品名称：{item.name}</p>
          <p>
            所需积分：<Text strong style={{ color: '#ff4d4f' }}>{item.points_required}</Text> 积分
          </p>
  ...
  const columns: ColumnsType<Redemption> = [
    ...
    },
    {
      ...
      render: (code: string) =>
        code ? (
          <Text copyable={{ tooltips: ['复制', '已复制'] }}>{code}</Text>
        ) : (
          '-'
        ),
    },
    {
      ...
      render: (status: string) => {
        const config = REDEMPTION_STATUS_MAP[status] || { label: status, color: 'default' };
  ...
  const renderItemsGrid = () => {
    ...
    if (activeItems.length === 0) {
      return <Empty description="暂无可兑换商品" />;
    }
              ...
              >
                  ...
                  <Space direction="vertical" size={4} style={{ width: '100%' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <Text strong style={{ color: '#ff4d4f', fontSize: 18 }}>
                        {formatPoints(item.points_required)}
                        <Text type="secondary" style={{ fontSize: 12, fontWeight: 'normal' }}>
                          {' '}积分
                        </Text>
                      </Text>
                      <Tag color={isOutOfStock ? 'red' : 'green'}>
                        {isOutOfStock ? '已兑完' : `剩余 ${item.stock}`}
                      </Tag>
                    </div>
                ...
                <div style={{ marginTop: 16 }}>
                  <Button
                    ...
                    block
                    disabled={isOutOfStock || isInsufficient || redeemMutation.isPending}
